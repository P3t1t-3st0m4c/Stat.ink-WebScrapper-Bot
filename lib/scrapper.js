...
const { localizations, strings } = require("./consts.json");
const { format, checkLocale } = require("./helpers")
const wordRegex = /^[a-z0-9]{6,}$/i

const retrieveHtml = async (url, loc) => {
    loc = loc in localizations ? localizations[loc] : "en-US"
    url = new URL(url)
...
const checkNameAndGetHtml = async (name, url, statloc, loc) => {
    let reg = name.match(wordRegex)
    reg === null ? reg = null : reg = reg[0]
    if (reg != name){
        return {"error" : ["RegexError", [name,wordRegex], format(strings[loc]["NameRegexError"], {name:name})]}
    }
    url = url(name)
    const html = await retrieveHtml(url, statloc)
    if (html == 404) return {"error" : ["404Error", [html,url,loc], format(strings[loc]["User404Error"], {name:name})]}
    return {"success" : {"html" : html, "url" : url}}

}

// Standard data
const retrieveStandardData = async (name, statloc, loc) => {
    let html = await checkNameAndGetHtml(name, (x) => `https://stat.ink/@${x}`, statloc, loc)
    ...
    try {
        ...
        splatoon2Data = [strings[loc].NoDataError, ""]
    }try{
        ...
        splatoonData = [strings[loc].NoDataError, ""]
    }
    return {"success": 
        {
...
const advancedDataSplat1 = async (name, statloc, loc) => {
    let html = await checkNameAndGetHtml(name, (x) => `https://stat.ink/@${x}/spl1`, statloc, loc)
    if (html.error) return html
    ...
    try {
        ...
        return {"error" : ["RetrieveError", error, strings[loc].RetrieveError]};
    }
}

const advancedDataSplat2 = async (name, statloc, loc) => {
    let html = await checkNameAndGetHtml(name, (x) => `https://stat.ink/@${x}/spl2`, statloc, loc)
    if (html.error) return html
...
const advancedDataSalmon = async (name, statloc, loc) => {
    let html = await checkNameAndGetHtml(name, (x) => `https://stat.ink/@${x}/salmon`, statloc, loc)
    if (html.error) return html
